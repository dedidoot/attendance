...
import gapara.co.id.databinding.ActivityFeedbackBinding
import gapara.co.id.feature.component.ReportView
import gapara.co.id.feature.feedback.CreateFeedbackActivity.Companion.createFeedbackCode
import kotlinx.android.synthetic.main.activity_emergency_report.*
import kotlinx.coroutines.launch
import java.util.*
import kotlin.collections.ArrayList

class FeedbackActivity : MvvmActivity<ActivityFeedbackBinding, FeedbackViewModel>(FeedbackViewModel::class), CoroutineDeclare {

    private val calendar = Calendar.getInstance()

    override val layoutResource: Int = R.layout.activity_feedback
    ...
    private fun setupAppBar() {
        ...
        appBarView.setTitleBar("Feedback")
    }
    ...
    private fun setupItemView(items: ArrayList<FeedbackModel>, rootView : LinearLayout) {
        launch {
            val size = items.size
            if (viewModel.feedbackListPage == 1) {
                rootView.removeAllViews()
            }
            items.forEachIndexed { index, model ->
                val itemView = ReportView(this@FeedbackActivity)
                ...
                itemView.setUsername(model.client?.name)
                itemView.setDescription(model.content)
                itemView.setClick {
                    openFeedbackDetail(model)
                }
    ...
    fun openCreateEmergencyReport() {
        startActivityForResult(CreateFeedbackActivity.onNewIntent(this), createFeedbackCode)
    }

    private fun openFeedbackDetail(model: FeedbackModel) {
        startActivity(FeedbackDetailActivity.onNewIntent(this, model))
        overridePendingTransition(R.anim.slide_up, R.anim.no_anim)
    }

    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == createFeedbackCode && resultCode == createFeedbackCode) {
            resetEmergencyList()
        } else {
    ...
    private fun loadFeedbackList() {
        viewModel.getFeedbackList(fromDate.formatServer, toDate.formatServer)
    }
